...
def clean_column_to_numeric(series):
    """Sostituisce testi non numerici con 0 e pulisce virgole."""
...
def load_and_process_data():
    try:
        ...
        db['Prod_Val'] = clean_column_to_numeric(db['World production (tons per year)'])
        db['Res_Val'] = clean_column_to_numeric(db['World reserve (tons)'])
        db['Risk_Val'] = clean_column_to_numeric(db['Supply risk'])
        db['HHI_Val'] = clean_column_to_numeric(db['HHI'])
        db['ESG_Val'] = clean_column_to_numeric(db['ESG'])

        # Costruzione Vettori Proprietà
        ...
        st.error(f"Errore critico durante l'elaborazione dei file CSV: {e}")
        return None
...
if df is not None:
    st.sidebar.markdown('<p class="settings-title">Global Settings</p>', unsafe_allow_html=True)
    manual_t = {'P1': [], 'P2': [], 'P3': []}
    is_valid = True
    ...
    with st.sidebar:
        ...
        for i in range(sf_t - 1):
            val = st.number_input(f"Score {sc_t[i+1]} Threshold (P1)", value=int(350+(i*50)), min_value=350, step=1, format="%d", key=f"p1_{i}")
        ...
        for p_label, p_key, def_idx, def_val in [("Magnetization", "P2", 1, 0.4), ("Coercivity", "P3", 3, 0.4)]:
            ...
            for i in range(sf - 1):
                v = st.number_input(f"Score {sc[i+1]} Threshold ({p_key})", value=def_val+(i*0.2), min_value=def_val, key=f"t_{p_key}_{i}")
                manual_t[p_key].append(v)
            if p_key == "P2": sf_m = sf
            else: sf_c = sf

        # Verifica ordine soglie
        for k in ['P1', 'P2', 'P3']:
            if any(manual_t[k][i] >= manual_t[k][i+1] for i in range(len(manual_t[k])-1)):
                st.error(f"Error: {k} thresholds must be ascending!")
                is_valid = False

        # SEZIONE 2: PESI PERFORMANCE
        ...
        # SEZIONE 3: MAPPA SCALABILITÀ
        st.markdown('<div class="blue-section-header"><p>3. Scalability View</p></div>', unsafe_allow_html=True)
        color_metric = st.selectbox("Color mapping metric:", 
                                    ["Sustainability (OSS)", "Supply Risk", "HHI", "ESG"])
        metric_map = {"Sustainability (OSS)": "OSS", "Supply Risk": "Calc_Supply_Risk", "HHI": "Calc_HHI", "ESG": "Calc_ESG"}
    ...
    if is_valid:
        ...
        with t2:
            ...
            fig_sc = px.scatter(df_plot, x='Calc_Reserves', y='Calc_Production', 
                                ...
                                hover_data=['Calc_Supply_Risk', 'Calc_HHI'],
            ...
            st.info("⭐ Stars indicate materials currently on the Pareto Frontier (Top Performers).")
    ...
    st.error("Assicurati di caricare 'AF_vectors.csv' e 'Materials Database 1.csv' nella stessa cartella.")
